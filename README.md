...
系统只读取、不修改 `users.csv`。文件默认使用 UTF-8 编码，包含表头，列顺序如下：

```csv
QQ号,学号,姓名,学院,群名片
10000001,20260001,张三,计算机科学与工程学院,张三-20260001
10000002,,李四,软件学院,李四
```

`app.py` 和 `users_csv_server.py` 按表头名称读取字段，并兼容 `群名片` 与 `QQ群名片` 两种列名。
...
- `admin_annotations`：按学号保存管理员添加的星标原因、批注内容、操作人和时间。
...
### `GET /api/admin/annotations/<学号>`

管理员查看指定报名人的星标和批注记录。响应中的每条记录包含 `content`、`operator` 和 `created_at`。

### `POST /api/admin/annotations/<学号>`

管理员添加星标或批注，必须携带 `X-CSRF-Token` 请求头。请求体示例：

```json
{"type": "star", "reason": "重点关注"}
```

或：

```json
{"type": "comment", "content": "安排复试"}
```

每类记录最多保存 500 条，单条内容最多 1000 个字符；接口会校验管理员权限、学号对应的报名记录和 CSRF token。
