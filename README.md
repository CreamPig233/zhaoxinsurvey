...
首次启动时自动创建 `data/survey.sqlite3`，包括以下表：
...
`departments` 至少包含一个部门，最多包含五个不重复的固定部门。`strengths_b64` 可以为空字符串；后端解码后限制为 4000 个字符。
...
导出内容包括 QQ 号、姓名、学号、性别、五个按优先级排列的部门志愿、调剂意愿、个人简介和填写时间。
...
- 后端重新校验姓名、学号、QQ 号、性别、部门顺序、调剂意愿和个人优势内容，不信任前端字段。
